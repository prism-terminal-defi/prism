...
// This file is a modified version of software developed by Radix Publishing Ltd.
// Original available at: https://github.com/radixdlt/Ignition/blob/main/libraries/scrypto-interface/src/handlers.rs
...
fn generate_scrypto_stub(
    ...
    .map(|ty| -> syn::Type {
        ...
        parse_quote!(::scrypto::prelude::#ty_ident)
    })
    .map(|ty| {
        quote! {
            #[allow(clippy::too_many_arguments)]
            ...
            {
                type Error = <
                    #ty as TryFrom<::scrypto::prelude::NodeId>
                >::Error;
            ...
            |Signature {
                ...
                let inner = if arguments.is_function() {
                    quote! {
                        ...
                        ::scrypto::prelude::scrypto_decode(&rtn).unwrap()
                    }
                } else {
                    quote! {
                        use ::sbor::Encoder;
             ...
             -> syn::Result<TokenStream2> {
                ...
                let inner = if arguments.is_function() {
                    ...
                    arguments.add_argument_to_beginning(
                        Ident::new("component_address", ident.span()),
                        parse_quote!(
                            impl ::radix_transactions::builder::ReferencedManifestGlobalAddress
                        ),
                    );
